...
// Initialise MQTT client & broker configuration 
int app_mqtt_init(struct mqtt_client *client);

// Blocking function that establishes connectivity to the MQTT broker 
void app_mqtt_connect(struct mqtt_client *client);

// Subscribes to user-defined MQTT topics and continuously processes incoming data while
// the MQTT connection is active
void app_mqtt_run(struct mqtt_client *client);

// Subscribe to user-defined MQTT topics
